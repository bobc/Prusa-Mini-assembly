...
module equiTriangle(side, height) {
  difference() 
  {
    ...
    rotate([0,0,30]) dislocateBox(side, side*1.5, height+1);
    translate([-side,0,0]) {
      rotate([0,0,60]) dislocateBox(side*1.5, side, height+1);
    }
  }
}
...
module 6pointStar(size, height) 
{
  starNum = 2;
  starAngle = 180;
  for (s = [0:starNum-1]) {
    rotate([0, 0, s*starAngle]) 
      translate ([size/2,-size/2/tan(60),0])
      equiTriangle(size, height);
  }
}
