...
module comp_spring(type, len = 0) 
{
    len = (len == 0) ? spring_length(type) : len;

    vitamin(str("SPR-", spring_od(type), "-", spring_gauge(type) * 100, "-", spring_length(type),
                ": Spring ", 
                spring_od(type), "mm OD, ", 
                spring_gauge(type), "mm gauge x ", 
                spring_length(type), "mm long" ));

    //color(spring_color) 
    render()
        coil(r1 = (spring_od(type) - spring_gauge(type)) / 2, 
          r2 = spring_gauge(type) / 2, 
          h = len, 
          twists = spring_coils(type));
